...
#define SEXT(val, bits) ((int64_t)((val) | (((val) & (1ull << ((bits) - 1))) ? ~((1ull << (bits)) - 1) : 0)))
...
static int decode_branch(uint32_t insn, arm64_insn_t *d) {
    ...
    if ((insn & 0xff000010) == 0x54000000) {
        ...
        switch (d->cond) {
            case 0x0: d->type = ARM64_INSN_BEQ; break;
            case 0x1: d->type = ARM64_INSN_BNE; break;
            case 0xa: d->type = ARM64_INSN_BGE; break;
            case 0xb: d->type = ARM64_INSN_BLT; break;
            case 0xc: d->type = ARM64_INSN_BGT; break;
            case 0xd: d->type = ARM64_INSN_BLE; break;
            case 0x8: d->type = ARM64_INSN_BHI; break;
            case 0x9: d->type = ARM64_INSN_BLS; break;
            default: return 0;
        }
        return 1;
    }
...
static int decode_load_store(uint32_t insn, arm64_insn_t *d) {
    ...
    if (op1 == 1 && op2 == 0 && op3 == 0) {
        ...
        if (opc == 0x1) {
            d->type = size == 3 ? ARM64_INSN_LDR_IMM : (size == 2 ? ARM64_INSN_LDR_IMM : (size == 1 ? ARM64_INSN_LDRH_IMM : ARM64_INSN_LDRB_IMM));
        } else if (opc == 0x0) {
            d->type = size == 3 ? ARM64_INSN_STR_IMM : (size == 2 ? ARM64_INSN_STR_IMM : (size == 1 ? ARM64_INSN_STRH_IMM : ARM64_INSN_STRB_IMM));
        } else {
            return 0;
        }
        return 1;
