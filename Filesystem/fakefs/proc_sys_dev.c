...
int devfs_stat(const char *path, struct stat *buf)
{
    ...
    if (strstr(path, "null") || strstr(path, "zero") || strstr(path, "random") ||
        strstr(path, "urandom")) {
...
ssize_t procfs_read(const char *path, char *buf, size_t size)
{
    if (strstr(path, "version")) {
        const char *version =
            "Linux version 5.15.0-termi (termi@ios) (gcc version 11.2.0) #1 SMP\n";
    ...
    if (strstr(path, "meminfo")) {
        const char *meminfo = "MemTotal:        2048000 kB\n"
                              "MemFree:         1024000 kB\n"
