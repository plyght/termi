...
bool fakefs_import(const char *archive_path, const char *fs_path, 
    ...
    sqlite3_prepare_v2(db, "INSERT OR REPLACE INTO paths VALUES (?, "
                          "(SELECT inode FROM paths WHERE path = ? LIMIT 1))", -1, &insert_hardlink, NULL);
...
bool fakefs_export(const char *fs_path, const char *archive_path, 
    ...
    sqlite3_stmt *query;
    sqlite3_prepare_v2(db, "SELECT path, inode, stat FROM paths, stats USING (inode)", -1, &query, NULL);
