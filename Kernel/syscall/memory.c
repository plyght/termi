...
static int translate_prot(int_t linux_prot) {
    ...
    if (linux_prot & PROT_READ_) darwin_prot |= PROT_READ;
    if (linux_prot & PROT_WRITE_) darwin_prot |= PROT_WRITE;
    if (linux_prot & PROT_EXEC_) darwin_prot |= PROT_EXEC;
...
static int translate_mmap_flags(dword_t linux_flags) {
    ...
    if (linux_flags & MAP_SHARED_) darwin_flags |= MAP_SHARED;
    if (linux_flags & MAP_PRIVATE_) darwin_flags |= MAP_PRIVATE;
    if (linux_flags & MAP_FIXED_) darwin_flags |= MAP_FIXED;
    if (linux_flags & MAP_ANONYMOUS_) darwin_flags |= MAP_ANON;
...
addr_t sys_brk(addr_t new_brk) {
    if (brk_base == NULL) {
        brk_base = mmap(NULL, 1024 * 1024 * 64, PROT_READ | PROT_WRITE,
